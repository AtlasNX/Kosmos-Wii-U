...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help='Overides the Wii U Kosmos Version from the config file.',
        metavar='KosmosVersion')
    subparsers = parser.add_subparsers()

    # Kosmos subcommands
    parser_kosmos = subparsers.add_parser('kosmos', help='Create a release build of Wii U Kosmos.')
    parser_kosmos.add_argument('output', help='Zip file to create.')
    parser_kosmos.set_defaults(command=common.Command.Kosmos)

    # SDSetup subcommands
    parser_sdsetup = subparsers.add_parser('sdsetup', help='Create the Wii U Kosmos modules for SDSetup.')
...
def init_version_messages(args, kosmos_version):
    if args.command == common.Command.Kosmos:
        return [ f'Wii U Kosmos {kosmos_version} built with:' ]
